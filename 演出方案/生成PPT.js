...
(() => {
  ...
  T(s, "朗诵与合唱有机融合 · 至少一个部分与长征有关 · 全程 5 分半以内", { x: 0.55, y: 4.62, w: 11.5, h: 0.45, fontSize: 15, color: C.dim });
...
(() => {
  ...
  const reqs = [
    ...
    ["命题要求 ③", "从上场到下场不超过 6 分钟", "自定 5′30″ 上限：朗诵 1′45″ ＋ 合唱 2′53″ ＋ 暗场/点题/定格 37″ ＝ 5′15″，留 15″ 余量。"],
  ];
  reqs.forEach((r, i) => {
    const x = M + i * 4.22;
...
(() => {
  ...
  const mech = [
    ...
    ["03", "尾声语录点题", "尾奏上齐诵总书记长征语录“走好自己的长征路”，注目礼定格，同台收束。"],
  ];
  mech.forEach((m, i) => {
    const x = M + i * 4.22;
  ...
  T(s, "时长账：朗诵 1′45″ ＋ 合唱 2′53″ ＋ 暗场/点题/定格 37″ ＝ 5′15″，与第 4 页时间轴一致。", { x: M, y: 6.62, w: 12.33, h: 0.35, fontSize: 12.5, color: C.muted });
  footer(s);
})();
...
(() => {
  const s = P.addSlide();
  header(s, "全流程 5′15″", "五分半时间轴");
  const segs = [ // [秒, 名称, 颜色, 文字色]
    [5, "暗场", C.dark2, C.white],
    [105, "朗诵《丰碑》 105″", C.red, C.white],
    [173, "合唱《精忠报国》 173″（齐诵转场＋间奏齐诵）", C.redDeep, C.white],
    [22, "点题", C.gold, C.ink],
    [10, "定格", "5A2A2E", C.white],
  ];
  ...
  // 小段名称放条上方
  T(s, "暗场 5″", { x: 0.5, y: 1.62, w: 1.6, h: 0.34, fontSize: 13, bold: true, color: C.ink });
  s.addShape("line", { x: 0.7, y: 1.96, w: 0, h: 0.14, line: { color: C.muted, width: 0.75 } });
  T(s, "点题 22″ · 定格 10″", { x: 10.63, y: 1.62, w: 2.2, h: 0.34, fontSize: 13, bold: true, color: C.ink, align: "right" });
  s.addShape("line", { x: 12.01, y: 1.96, w: 0, h: 0.14, line: { color: C.muted, width: 0.75 } });
  // 时间刻度（0:05 与 0:00 仅差5秒，错一行避免重叠）
  const ticks = [["0:00", 0.5, "left"], ["1:50", 4.806, "left"], ["4:43", 11.578, "left"]];
  ticks.forEach(tk => T(s, tk[0], { x: tk[1] - 0.5, y: 3.0, w: 1.0, h: 0.3, fontSize: 12, color: C.muted, align: tk[2] }));
  [["0:05", 0.696], ["5:15", 12.634]].forEach(tk => {
  ...
  // 五列说明
  const cols = [
    ["① 暗场 0:00–0:05", "风雪声渐起，LED雪山剪影亮起。"],
    ["② 朗诵 0:05–1:50", "精简稿 1′45″；群诵两次插入。"],
    ["③ 合唱 1:50–4:43", "前奏齐诵转场；间奏齐诵20″点题。"],
    ["④ 点题 4:43–5:05", "总书记长征语录，领诵＋全体。"],
    ["⑤ 定格 5:05–5:15", "注目礼定格5″，音乐收。"],
  ];
  cols.forEach((c, i) => {
    const x = M + i * 2.466;
  ...
  T(s, "彩排验收：全程掐表 5′15″ ± 10″", { x: 5.45, y: 6.24, w: 5.2, h: 0.45, fontSize: 20, bold: true, color: C.red });
  s.addNotes("时间轴按秒对齐预录音轨：风雪5″、朗诵105″、合唱173″（齐诵转场与间奏齐诵含在内）、点题22″、定格10″，总长5′15″。");
  footer(s);
})();
...
(() => {
  ...
  s.addTable(data, tbOpt(M, 1.72, [1.35, 7.6, 3.38], [0.36, 0.44, 0.44, 0.6, 0.44, 0.6, 0.44]));
  T(s, "改编自李本深《丰碑》（统编课文，又名《军需处长》）；为卡进 5 分半，删去“让马”“思索”“神态”三句与一次群诵。领诵站位见第 9 页队形图。", { x: M, y: 6.6, w: 12.33, h: 0.35, fontSize: 12, color: C.muted });
  s.addNotes("群诵=合唱四排齐声。精简稿约260字，按领诵2.5字/秒＋气口约1′45″；女领②在朗诵段先于群诵位置待命。");
  footer(s);
})();
...
(() => {
  ...
  const rows = [
    ...
    ["男领②", "将军愣住了。他深深吸了一口气，缓缓举起右手——向军需处长敬了一个军礼！", "随句敬礼，全体注目"],
    ["女领②", "风更狂了，雪更大了。大雪很快覆盖了他——他，成了一座晶莹的丰碑！", "“晶莹的丰碑”一字一顿"],
    ["群诵", "晶莹的——丰碑——", "回声式，由弱到强"],
    ["男领①", "如果胜利不属于这样的队伍，还会属于谁呢？！", "层层推高，问句砸实"],
    ["全体", "红军不怕远征难，万水千山只等闲！", "最强音；《精忠报国》鼓点从此句压进"],
  ];
  const data = [[hd("角色"), hd("台词"), hd("表演处理")]];
  rows.forEach(r => data.push([role(r[0]), say(r[1]), tip(r[2])]));
  s.addTable(data, tbOpt(M, 1.72, [1.35, 7.6, 3.38], [0.36, 0.44, 0.44, 0.6, 0.6, 0.44, 0.44, 0.44]));
  rect(s, { x: 0, y: 5.75, w: W, h: 0.85, fill: { color: C.tint } });
  T(s, "转场：末句“红军不怕远征难”（毛泽东《七律·长征》）压《精忠报国》前奏战鼓——鼓点在“万水千山”处预进，收字即全体起势，无缝接唱。",
    { x: 0.7, y: 5.75, w: 11.93, h: 0.85, fontSize: 13, color: C.ink, valign: "middle" });
...
(() => {
  ...
  s.addText([
    { text: "剪去主歌二（省约 40″），为结尾语录点题让出时间，全曲约 2′53″；", options: { bullet: { code: "25B8", indent: 12 }, breakLine: true } },
    { text: "尾句“堂堂中国要让四方来贺”渐慢渐强，唱毕接“结尾点题”（第 8 页）；", options: { bullet: { code: "25B8", indent: 12 }, breakLine: true } },
    { text: "合唱队形全程不动，领诵朗诵段进前区、点题段再上前一步。", options: { bullet: { code: "25B8", indent: 12 } } },
  ...
  s.addText([
    { text: "主歌一：男领唱独唱（麦3）；副歌：领唱带唱，四排合唱铺底；", options: { bullet: { code: "25B8", indent: 12 }, breakLine: true } },
    { text: "女声部二声部视排练水平加花；间奏齐诵由合唱四排完成。", options: { bullet: { code: "25B8", indent: 12 } } },
  ], { x: 5.6, y: 6.35, w: 7.23, h: 0.9, fontFace: F, fontSize: 13, color: C.ink, paraSpaceAfter: 6, margin: 0 });
  s.addNotes("主歌二已剪去，让位结尾点题（第8页）；间奏齐诵务必卡在20秒内收字，收字落在副歌前一拍。");
  footer(s);
})();
...
(() => {
  const s = P.addSlide();
  header(s, "2016·10·21 纪念红军长征胜利80周年大会讲话", "结尾点题：走好我们的长征路");
  rect(s, { x: 0, y: 1.78, w: W, h: 0.8, fill: { color: C.tint } });
  T(s, "音乐：尾句唱毕尾奏不停，弦乐垫渐起；LED 转金色点题大字“走好我们这一代人的长征路”。本段约 22″。",
    { x: 0.7, y: 1.78, w: 11.93, h: 0.8, fontSize: 12.5, color: C.ink, valign: "middle" });
  const rows = [
    ["男领②", "长征，永远在路上。", "庄重起句，一字一顿"],
    ["女领①", "一个不记得来路的民族，是没有出路的民族。", "沉痛而清醒"],
    ["女领②", "每一代人有每一代人的长征路——", "层层扬起"],
    ["全体", "每一代人，都要走好自己的长征路！", "最强音；收字即注目礼定格"],
  ];
  const data = [[hd("角色"), hd("台词"), hd("表演处理")]];
  rows.forEach(r => data.push([role(r[0]), say(r[1]), tip(r[2])]));
  s.addTable(data, tbOpt(M, 2.85, [1.35, 7.6, 3.38], [0.36, 0.52, 0.52, 0.52, 0.52]));
  const boxes = [
    ["替换句", "“心中有信仰，脚下有力量。”（同出此讲话）——可替换第二句，省时更利落；LED 大字随之更换。"],
    ["语录出处", "三句均引自习近平总书记《在纪念红军长征胜利80周年大会上的讲话》（新华社受权发布），报幕词可直接引用。"],
  ];
  boxes.forEach((b, i) => {
    const x = M + i * 4.22;
    rrect(s, { x, y: 5.45, w: 3.89, h: 1.35, rectRadius: 0.06, fill: { color: C.white }, line: { color: C.line, width: 1 } });
    T(s, b[0], { x: x + 0.24, y: 5.58, w: 3.45, h: 0.34, fontSize: 14, bold: true, color: C.red });
    T(s, b[1], { x: x + 0.24, y: 5.94, w: 3.45, h: 0.8, fontSize: 12, color: C.ink });
  });
  s.addNotes("点题四句共约40字，按2字/秒＋气口约22秒。彩排若超时，删女领①第二句，直接“每一代人……”接全体。");
  footer(s);
})();
...
(() => {
  ...
  T(s, "朗诵段上前区；歌曲段退回一排两翼；点题段再上前一步收束", { x: 6.6, y: 1.72, w: 5.8, h: 0.32, fontSize: 12.5, color: C.muted, align: "right" });
  // 四排合唱（下=前）
...
(() => {
  ...
  const steps = [
    ...
    ["3", "混音单轨", "总长 5′15″，首尾各留 2″ 静音；U盘＋手机双备份。"],
...
(() => {
  ...
  const cards = [
    ...
    ["LED 与音效", [
      "背景五段：风雪山林→雪山日出→红旗长城→丰碑剪影→金色点题大字",
      "音效：风雪声 15″ 渐入＋战鼓",
      "背景素材见第 12 页资源表",
      "免费音效：爱给网 aigei.com",
    ]],
...
(() => {
  ...
  const rows = [
    ...
    ["总书记长征讲话全文", "结尾点题出典·报幕词", "新华社受权发布：纪念红军长征胜利80周年大会上的讲话"],
  ];
  const data = [[hd("作品"), hd("学什么"), hd("在哪看")]];
  rows.forEach(r => data.push([say(r[0]), tip(r[1]), tip(r[2])]));
  s.addTable(data, tbOpt(M, 1.78, [3.5, 3.0, 5.83], [0.36, ...Array(10).fill(0.42)]));
...
(() => {
  ...
  const weeks = [
    ...
    ["第 3 周", "合成 · 录音", ["朗诵×合唱拼接合乐", "进场录对口型音轨", "全程 5′15″±10″"]],
...
(() => {
  ...
  T(s, "走好我们这一代人的长征路——预祝演出成功！", { x: 1.0, y: 4.95, w: 11, h: 0.5, fontSize: 19, bold: true, color: C.goldSoft });
  T(s, "《精忠·丰碑》节目组 · 2026 年 9 月", { x: 1.0, y: 6.45, w: 8, h: 0.4, fontSize: 13, color: C.dim });
  footer(s);
})();

P.writeFile({ fileName: process.argv[2] || "精忠丰碑-演出设计方案.pptx" }).then(f => console.log("written:", f));
